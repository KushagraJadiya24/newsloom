...
const STORIES = [
  {
    category: "MARKETS",
    headline:
      "Sensex surges 800 points as FII inflows hit 3-month high amid global risk-on rally",
    summary:
      "Foreign institutional investors pour capital into Indian equities as global sentiment improves.",
  },
  {
    category: "POLICY",
    headline:
      "RBI holds repo rate at 6.5% for fifth straight meeting; signals possible cut in Q1 2026",
    summary:
      "Central bank maintains status quo while inflation softens to 4.2% in latest readings.",
  },
  {
    category: "TECH",
    headline:
      "Reliance Jio launches AI-powered 6G trials in Mumbai and Bengaluru corridors",
    summary:
      "India's largest telecom operator begins next-gen network testing in two metro cities.",
  },
  {
    category: "STARTUP",
    headline:
      "Indian startup funding rebounds to $3.2B in Q3 2025 led by fintech and deeptech bets",
    summary:
      "Venture capital returns to Indian startups after a prolonged funding winter.",
  },
  {
    category: "ECONOMY",
    headline:
      "India's GDP growth forecast upgraded to 7.2% by IMF on strong domestic consumption",
    summary:
      "International Monetary Fund raises India outlook citing robust consumer spending.",
  },
  {
    category: "CORPORATE",
    headline:
      "Tata Motors eyes $2B EV plant in Pune as government extends PLI scheme benefits",
    summary:
      "Automaker plans major electric vehicle manufacturing facility under government incentives.",
  },
];
        ...
        <div className="flex flex-wrap justify-center gap-2 mt-4">
          {TOPIC_CHIPS.map((chip) => (
            <button
              ...
              onClick={() => {
                setQuery(chip);
                navigate(`/briefing?topic=${encodeURIComponent(chip)}`);
              }}
      ...
      <section className="max-w-5xl mx-auto px-6 mt-16 pb-16">
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-5">
          {STORIES.map((story, i) => (
            <article
              key={i}
              className="bg-card border border-border rounded-md p-5 flex flex-col justify-between"
            >
              <div>
              ...
              <button
                onClick={() =>
                  navigate(
                    `/briefing?topic=${encodeURIComponent(story.headline)}`,
                  )
                }
